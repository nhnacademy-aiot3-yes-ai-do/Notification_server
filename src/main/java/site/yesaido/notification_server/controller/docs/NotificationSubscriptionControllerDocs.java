...
public interface NotificationSubscriptionControllerDocs {
    ...
    ResponseEntity<SubscriptionResponse> create(Long userId, SubscriptionCreateRequest request);

    @Operation(summary = "내 구독 목록", description = "요청자의 알림 구독 목록을 반환합니다.")
    ResponseEntity<List<SubscriptionResponse>> findAll(Long userId);

    @Operation(summary = "구독 사용/일시정지 전환", description = "`enabled` 값으로 구독을 일시정지/재개합니다.")
    ResponseEntity<SubscriptionResponse> changeEnabled(
            Long userId,
            @Parameter(description = "구독 ID") Long subscriptionId,
            SubscriptionEnabledRequest request);

    @Operation(summary = "구독 해지", description = "구독을 소프트 삭제합니다.")
    @ApiResponse(responseCode = "204", description = "해지됨")
    ResponseEntity<Void> delete(Long userId, @Parameter(description = "구독 ID") Long subscriptionId);
}
