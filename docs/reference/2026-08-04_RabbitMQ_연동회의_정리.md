...
둘 다 Durable로 선언한다. 이후 2026년 8월 11일 팀 합의로 DLX는 공용 Queue 하나에
실패 메시지를 모으기 위한 Fanout Exchange로 최종 변경됐다.
...
2026년 8월 4일 회의 시점에는 Routing Key 문자열이 미확정이었다. 이후 2026년 8월 11일
Notification 계약에서 Direct Exchange의 Routing Key를 각 Queue 이름과 동일하게 사용하기로
확정했다. 이 값은 더 이상 환경변수 기본값이 아니라 `NotificationRabbitConstants`의 코드
상수다.
...
Routing Key: yes-nhn.notification.action.queue
```
...
- [x] Routing Key 문자열 확정 (Queue 이름과 동일)
