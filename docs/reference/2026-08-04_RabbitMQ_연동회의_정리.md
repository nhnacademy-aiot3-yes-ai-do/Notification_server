...
Producer별 최종 JSON payload는 후속 합의 항목으로 남는다. 이후 팀 결정으로 Notification
Exchange의 Routing Key는 각 소비 Queue명과 동일하게 고정했고, 공용 DLX는 Fanout으로 확정했다.
...
| `yes-nhn.dlx` | Fanout | Routing Key 없이 공용 DLQ로 실패 메시지를 전달 |
...
Notification의 Exchange·Queue·Routing Key는 `NotificationRabbitConstants`에서 고정 관리한다.
RabbitMQ 접속 주소와 인증 정보만 로컬 `.env` 또는 배포 Secret으로 관리하며, 기본 vhost(`/`)를
사용하므로 별도 vhost 설정은 두지 않는다.
